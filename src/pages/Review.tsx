...
export default function Review() {
  ...
  const [reviewerName, setReviewerNameInput] = useState(storedReviewerName || '');
  ...
  const inspectionDone = inspectionStats.confirmed === inspectionStats.total;
  const rollcallDone = rollcallStats.pending === 0;
  const canReview = inspectionDone && rollcallDone;
          ...
          <div className="grid grid-cols-2 gap-8">
            ...
            <div className="space-y-8">
              <div className="card-base">
                ...
                {pendingChildren.length > 0 && (
                  <div className="p-4 bg-danger-50 rounded-2xl border-2 border-danger-200">
                    <p className="text-danger-700 font-bold text-lg mb-3 flex items-center gap-2">
                    ...
                    <div className="flex flex-wrap gap-2 max-h-40 overflow-y-auto scrollbar-thin">
                      {pendingChildren.map((c) => {
                        ...
                        return (
                          <span
                            key={c.id}
                            className="inline-flex items-center gap-2 px-4 py-2 bg-white rounded-2xl text-danger-600 font-bold text-lg shadow-sm border border-danger-200"
                          >
                            <span
                            ...
                            />
                            {c.name}
                            <span className="text-gray-400 text-sm font-medium">{cls?.name}</span>
                          </span>
                        );
                      })}
                    </div>
              ...
              <div className="card-base">
                ...
                {!canReview && (
                  <div className="mt-4 p-4 bg-danger-50 rounded-2xl border-2 border-danger-200">
                    {!inspectionDone && (
                      <p className="text-danger-600 font-bold text-lg flex items-center gap-2 mb-2">
                        <AlertTriangle size={22} />
                        离车检查未完成：还有 {inspectionStats.total - inspectionStats.confirmed} 个区域未确认
                      </p>
                    )}
                    {!rollcallDone && (
                      <p className="text-danger-600 font-bold text-lg flex items-center gap-2">
                        <AlertTriangle size={22} />
                        儿童交接未完成：还有 {rollcallStats.pending} 名儿童待交接
                      </p>
                    )}
                  </div>
                )}
